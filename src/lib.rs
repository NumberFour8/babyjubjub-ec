...
//! - **Validation.** Points decoded via
//!   [`GroupEncoding::from_bytes`](group::GroupEncoding::from_bytes) are
//!   checked to be on-curve **and** in the prime-order subgroup. The
//!   [`GroupEncoding::from_bytes_unchecked`](group::GroupEncoding::from_bytes_unchecked)
//!   decoder performs **no** such checks; prefer
//!   [`AffinePoint::new`] or the validation helpers
//!   ([`AffinePoint::is_on_curve`], [`AffinePoint::is_in_prime_order_subgroup`])
//!   for untrusted input.
//! - **Canonical encodings.** [`Scalar::from_bytes`] /
//!   [`PrimeField::from_repr`](group::ff::PrimeField::from_repr)
...
//!   by [`CurveArithmetic`](elliptic_curve::CurveArithmetic)), so you can call
...
// `group` (and the `ff` traits it re-exports) is only present with the curve
// arithmetic, so this re-export is gated on the `arithmetic` feature.
#[cfg(feature = "arithmetic")]
pub use group;
// Re-exported so downstream users can reach the RFC 9380 hash-to-curve traits
// (`MapToCurve`, `GroupDigest`) and the `hash_from_bytes` / `encode_from_bytes` /
// `hash_to_scalar` routines without taking a separate dependency. The whole
// hash-to-curve surface — including this re-export — is gated behind the
// `hash2curve` feature.
#[cfg(feature = "hash2curve")]
pub use hash2curve;
pub use subtle;

// The marker traits `Curve` / `PrimeCurve` are available from `elliptic-curve`
// without its `arithmetic` feature, so they (and the impls below) live at the
// crate root. Everything that needs `CurveArithmetic` lives in the
// `arithmetic` module further down.
...
// Only referenced by `Scalar`'s canonical-encoding checks (arithmetic).
#[cfg(feature = "arithmetic")]
...
// ===== Curve arithmetic =====
//
// Everything below — the `CurveArithmetic` impl, the `AffinePoint` /
// `ProjectivePoint` / `Scalar` wrapper types and all of their `group`/`ff` trait
// impls, the `GroupEncoding`, and (behind the additional `hash2curve` feature)
// the RFC 9380 hash-to-curve machinery — is gated on the `arithmetic` feature.
// It lives in this private module and is re-exported (`pub use arithmetic::*`)
// at the crate root, so the public item paths are unchanged.
#[cfg(feature = "arithmetic")]
pub use arithmetic::*;
...
mod arithmetic {
    ...
    // always-available re-export of the `zeroize` crate. `CurveArithmetic`
    // requires `Scalar`/`AffinePoint`/`ProjectivePoint: DefaultIsZeroes` without
    // any feature gate, so this is not tied to the optional local `zeroize`
    // feature.
    ...
    use elliptic_curve::ops::{
        Invert, LinearCombination, MulByGeneratorVartime, MulVartime, Reduce,
    };
    use elliptic_curve::point::{AffineCoordinates, NonIdentity};
    use elliptic_curve::scalar::{FromUintUnchecked, IsHigh};
    use elliptic_curve::{
        BatchNormalize, CurveAffine, CurveGroup, Error as EcError, FieldBytes, Generate,
        ScalarValue,
    };
    use rand_core::TryCryptoRng;
    // ===== Imports for hash-to-curve (`MapToCurve` / `GroupDigest`, RFC 9380) =====
    // Gated together with the rest of the hash-to-curve code on `hash2curve`.
    // `MontCurveConfig` exposes the backend's Montgomery-form coefficients (A, B)
    // used by the Elligator2 map; `BigInteger` provides `is_odd` for `sgn0`;
    // `MapToCurve` is the curve-map trait from the standalone `hash2curve` crate.
    #[cfg(feature = "hash2curve")]
    ...
    // The `GroupDigest` convenience trait must commit to a concrete expander/hash.
    #[cfg(feature = "hash2curve")]
    ...
    impl ProjectivePoint {
        ...
        // Group doubling via the backend's complete (exception-free) formula.
        // Equivalent to `Group::double` now that the arithmetic is total; kept as an
        // explicit internal helper. For internal/test use only.
        #[allow(dead_code)]
        pub(crate) fn double_unchecked(&self) -> Self {
            self.to_backend_unvalidated().double().into()
        }

        // Scalar multiplication via the backend's complete formula. Equivalent to
        // the `*` operator now that the arithmetic is total; kept as an explicit
        // internal helper. For internal/test use only.
        #[allow(dead_code)]
        ...
        pub fn is_identity(&self) -> bool {
            ...
            let x_is_zero =
                limbs_x[0] == 0 && limbs_x[1] == 0 && limbs_x[2] == 0 && limbs_x[3] == 0;
        ...
        /// # Cofactor
        ///
        /// Uses the complete (exception-free) group law, so it works for **any**
        /// on-curve `self` — including torsion / small-subgroup points — and never
        /// panics. Like the `*` operator it does **not** clear the cofactor: if
        /// `self` carries a torsion component so does the result. When the result
        /// must lie in the prime-order subgroup (e.g. `self` came from untrusted
        /// input) use [`ProjectivePoint::mul_with_cofactor_clear`] instead.
        ///
        /// # Timing
        ///
    ...
    //
    // `group::prime::PrimeGroup` is spelled out in full to avoid clashing with the
    // already-imported `ark_ec::PrimeGroup`.
    ...
    impl group::cofactor::CofactorGroup for ProjectivePoint {
        ...
        fn clear_cofactor(&self) -> Self::Subgroup {
            // [8]P = [2]([2]([2]P)) via the complete doubling formula.
    ...
    // ===== Hash-to-curve (`MapToCurve` / `GroupDigest`, RFC 9380) =====
    ...
    /// BabyJubJub curve (the [`MapToCurve::FieldElement`](hash2curve::MapToCurve)
    /// associated type).
    ///
    ...
    /// callers (e.g. the `hash2curve` routines) compose this with
    ...
    /// RFC 9380 `L` for the BabyJubJub base/scalar fields:
    /// `L = ceil((ceil(log2 q) + 128) / 8) = ceil((254 + 128) / 8) = 48` bytes
    /// (the scalar field gives the same `L`, `ceil((251 + 128) / 8) = 48`).
    #[cfg(feature = "hash2curve")]
    type Okm = elliptic_curve::array::Array<u8, elliptic_curve::consts::U48>;

    #[cfg(feature = "hash2curve")]
    impl Reduce<Okm> for FieldElement {
        /// Interprets the 48 bytes of expander output as a big-endian integer and
        /// reduces it modulo the base-field prime `q` (RFC 9380 `OS2IP` then reduce).
        fn reduce(value: &Okm) -> Self {
            FieldElement(BackendBaseField::from_be_bytes_mod_order(&value[..]))
        }
    }

    #[cfg(feature = "hash2curve")]
    impl Reduce<Okm> for Scalar {
        /// Interprets the 48 bytes of expander output as a big-endian integer and
        /// reduces it modulo the scalar-field prime `r`. Enables hashing to a scalar
        /// via [`hash2curve::hash_to_scalar`].
        fn reduce(value: &Okm) -> Self {
            Scalar(BackendScalar::from_be_bytes_mod_order(&value[..]))
        }
    }

    /// The curve-specific hash-to-curve map for BabyJubJub (twisted-Edwards
    /// Elligator2). Gated, with the rest of the hash-to-curve surface, on the
    /// `hash2curve` feature. It does **not** commit to any hash function, so
    /// callers pick the expander themselves via the `hash2curve` free functions,
    /// e.g.
    /// `hash2curve::hash_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(&[msg], &[dst])`.
    #[cfg(feature = "hash2curve")]
    impl MapToCurve for BabyJubJub {
        // 128-bit target security level (in bytes), per RFC 9380 §8.9. With
        // `ExpandMsgXmd<Sha256>` this requires `Sha256::OutputSize (32) >= 2·16`.
        type SecurityLevel = elliptic_curve::consts::U16;
        type FieldElement = FieldElement;
        type Length = elliptic_curve::consts::U48;

        fn map_to_curve(element: Self::FieldElement) -> ProjectivePoint {
            map_to_curve_elligator2(element.0)
        }
    }

    /// Hash arbitrary byte strings to BabyJubJub points per RFC 9380, committing to
    /// the `expand_message_xmd` expander over SHA-256.
    ///
    /// Part of the `hash2curve`-gated surface (which pulls in `sha2`). The
    /// prime-order group used by `hash_from_bytes` / `encode_from_bytes` is the
    /// cofactor-cleared subgroup (see [`CofactorGroup`](group::cofactor::CofactorGroup)).
    /// This trait fixes the expander to `ExpandMsgXmd<Sha256>`, so you can call
    /// `BabyJubJub::hash_from_bytes(&[msg], &[dst])` directly; to use a different
    /// expander, call the expander-generic free functions in the `hash2curve`
    /// crate together with the [`MapToCurve`] impl above (both likewise behind the
    /// `hash2curve` feature).
    ///
    ...
    /// canonical labels — the actual domain separation tag is the caller-supplied
    /// `dst`.
    #[cfg(feature = "hash2curve")]
    impl GroupDigest for BabyJubJub {
        const HASH_TO_CURVE_ID: &'static [u8] = b"BabyJubJub_XMD:SHA-256_ELL2_RO_";
        const ENCODE_TO_CURVE_ID: &'static [u8] = b"BabyJubJub_XMD:SHA-256_ELL2_NU_";
    ...
    // ===== serde support =====
    //
    // `Scalar` and `AffinePoint` (de)serialize through `serdect`, so the
    // canonical byte encoding is rendered as lowercase hex for human-readable
    // formats (JSON, TOML, …) and as raw bytes for binary formats. The byte
    // encodings are the crate's existing canonical ones, and both decoders
    // reject malformed / non-canonical / out-of-subgroup input with a serde
    // error instead of yielding an invalid value:
    //
    // - `Scalar`: the 32-byte big-endian `to_bytes` / `from_bytes` encoding,
    //   which rejects non-canonical (`>= r`) values on the way in.
    // - `AffinePoint`: the 32-byte compressed `GroupEncoding` used by
    //   `ProjectivePoint`; decoding validates that the point is on-curve and in
    ...
    impl<'de> serde::Deserialize<'de> for Scalar {
            ...
            D: serde::Deserializer<'de>,
        {
            ...
            // `serdect`'s hex (human-readable) path does not enforce a length, so
            // a short hex string would otherwise decode into a zero-padded prefix
            // of the buffer; require the full length explicitly. (The binary path
            // already enforces an exact length.)
            let len = serdect::array::deserialize_hex_or_bin(&mut bytes, deserializer)?.len();
            if len != Self::SIZE {
                return Err(D::Error::custom(
                    "invalid BabyJubJub scalar: expected 32 bytes",
                ));
            }
            // `from_bytes` rejects non-canonical (`>= r`) encodings.
            let decoded: Option<Self> = Self::from_bytes(&bytes).into();
            decoded.ok_or_else(|| {
                D::Error::custom("invalid BabyJubJub scalar: non-canonical encoding (>= r)")
            })
        }
    }
    ...
    impl serde::Serialize for AffinePoint {
        ...
        where
            S: serde::Serializer,
        {
            // Canonical 32-byte compressed encoding (same as `GroupEncoding`).
            let repr = ProjectivePoint::from(self).to_bytes();
            serdect::array::serialize_hex_lower_or_bin(&repr, serializer)
        }
    }
    ...
    impl<'de> serde::Deserialize<'de> for AffinePoint {
        ...
        {
            use serde::de::Error;
            let mut bytes = [0u8; GroupRepr::SIZE];
            // See the note on `Scalar`'s decoder: enforce the exact length so the
            // lenient hex path cannot accept a zero-padded short input.
            let len = serdect::array::deserialize_hex_or_bin(&mut bytes, deserializer)?.len();
            if len != GroupRepr::SIZE {
                return Err(D::Error::custom(
                    "invalid BabyJubJub point: expected 32 bytes",
                ));
            }
            // Validated decode (on-curve + prime-order subgroup). `to_affine` is
            // panic-free here because a successfully decoded point has `z != 0`.
            let decoded: Option<ProjectivePoint> =
                ProjectivePoint::from_bytes(&GroupRepr(bytes)).into();
            decoded.map(|p| p.to_affine()).ok_or_else(|| {
                D::Error::custom(
                    "invalid BabyJubJub point: malformed, off-curve, or not in the prime-order subgroup",
                )
            })
        }
    }
    ...
    impl From<ProjectivePoint> for BackendProjective {
        /// Converts to the backend's extended twisted-Edwards representation using
        /// the **complete** (exception-free) coordinate map, accepting *any*
        /// on-curve point — including torsion / small-subgroup points. It never
        /// panics on subgroup membership.
        ///
        /// This totality is what makes the arithmetic operators (`+`, `-`, `*`,
        /// [`Group::double`], [`ProjectivePoint::mul_fixed_schedule`]) compute the
        /// genuine twisted-Edwards group law for all on-curve inputs — required for
        /// a sound [`Group`] / [`CofactorGroup`](group::cofactor::CofactorGroup)
        /// impl and for RFC 9380 hash-to-curve (which adds raw, possibly-torsion
        /// mapped points before clearing the cofactor). Security corollary: plain
        /// `*` does **not** clear the cofactor, so a result derived from an
        /// attacker-controlled point can carry a torsion component — use
        /// [`ProjectivePoint::mul_with_cofactor_clear`] or
        /// [`CofactorGroup::clear_cofactor`](group::cofactor::CofactorGroup::clear_cofactor)
        /// when a prime-order-subgroup result is required.
        fn from(point: ProjectivePoint) -> Self {
            // `to_backend_unvalidated` performs the inversion-free
            // `(x, y, z) -> (x·z, y·z, x·y, z²)` map, a valid extended representation
            // for every point (the identity maps to the backend's `zero()`), so no
            // subgroup assertion is performed.
            point.to_backend_unvalidated()
        }
    }
    ...
    mod tests {
        ...
        fn test_cofactor_clearing_security() {
            ...
            // We use struct instantiation (no on-curve / subgroup checks); the
            // arithmetic operators are total, so `*` / `Group::double` work here too.
            ...
            //
            // `mul_unchecked` is equivalent to the `*` operator here (the arithmetic
            // is total); using it keeps the small-subgroup intent explicit.
        ...
        // ===== Cofactor group (`CofactorGroup`) tests =====
        ...
        fn test_cofactor_group_trait() {
            ...
            // subgroup. Built via a struct literal so no subgroup assertion is fired.
            let p2 = ProjectivePoint {
                x: BackendBaseField::ZERO,
                y: -BackendBaseField::ONE,
                z: BackendBaseField::ONE,
        ...
        // ===== Hash-to-curve (`MapToCurve` / `GroupDigest`, RFC 9380) tests =====
        //
        // These exercise the `hash2curve`-gated items above, so the whole block is
        // gated on the same feature and grouped into a submodule. (`super::*` reaches
        // the parent test module, which itself re-exports the crate's arithmetic.)
        #[cfg(feature = "hash2curve")]
        mod hash2curve_tests {
            use super::*;
            ...
            fn test_map_to_curve_matches_arkworks() {
                ...
                <Elligator2Map<Bjj> as ArkMapToCurve<ArkProjective<Bjj>>>::check_parameters()
                    .unwrap();
                ...
                for i in 0u64..256 {
                    ...
                    let reference =
                        <Elligator2Map<Bjj> as ArkMapToCurve<ArkProjective<Bjj>>>::map_to_curve(u)
                            .unwrap();
                    // In `hash2curve` 0.14 the map is an associated fn on the curve type
                    // (it was a method on the field element in elliptic-curve 0.13).
                    let ours =
                        <BabyJubJub as hash2curve::MapToCurve>::map_to_curve(FieldElement(u));
            ...
            fn test_map_then_clear_cofactor_in_subgroup() {
                ...
                for i in 0u64..64 {
                    let p = <BabyJubJub as hash2curve::MapToCurve>::map_to_curve(FieldElement(
                        BackendBaseField::from(i),
                    ));
            ...
            /// End-to-end hash-to-curve through the expander-generic `hash2curve` free
            /// functions (no `hash2curve` feature needed): the result is a
            /// deterministic, on-curve, prime-order-subgroup point, and distinct
            /// messages give distinct points.
            #[test]
            fn test_hash_from_bytes_free_fn_properties() {
                ...
                let p =
                    hash2curve::hash_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(&[m], &[dst])
                        .unwrap();
                ...
                let q_msg: &[u8] = b"abd";
                let q = hash2curve::hash_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(
                    &[q_msg],
                    &[dst],
                )
                .unwrap();
            ...
            #[test]
            fn test_encode_from_bytes_free_fn_in_subgroup() {
                use hash2curve::ExpandMsgXmd;
                use sha2::Sha256;
                ...
                let p =
                    hash2curve::encode_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(&[m], &[dst])
                        .unwrap();
                assert!(p.is_on_curve());
                assert!(p.is_in_prime_order_subgroup());
            }

            /// `hash_to_scalar` (free function, `L = 48`) is deterministic and
            /// domain-separated.
            #[test]
            fn test_hash_to_scalar_free_fn() {
                ...
                let s = hash2curve::hash_to_scalar::<BabyJubJub, ExpandMsgXmd<Sha256>, U48>(
                    &[m],
                    &[dst],
                )
                .unwrap();
                let s_again = hash2curve::hash_to_scalar::<BabyJubJub, ExpandMsgXmd<Sha256>, U48>(
                    &[m],
                    &[dst],
                )
                .unwrap();
                ...
                let s_other = hash2curve::hash_to_scalar::<BabyJubJub, ExpandMsgXmd<Sha256>, U48>(
                    &[m],
                    &[dst2],
                )
                .unwrap();
            ...
            /// With the `hash2curve` feature on, the `GroupDigest` trait fixes the
            /// expander to `ExpandMsgXmd<Sha256>`, so `BabyJubJub::hash_from_bytes`
            /// matches the corresponding free-function call. (Already inside the
            /// `hash2curve`-gated module, so no extra `#[cfg]` is needed.)
            #[test]
            fn test_group_digest_trait_matches_free_fn() {
                use hash2curve::{ExpandMsgXmd, GroupDigest};
                use sha2::Sha256;

                let dst: &[u8] = b"BabyJubJub_XMD:SHA-256_ELL2_RO_";
                let m: &[u8] = b"group digest";

                let via_trait = BabyJubJub::hash_from_bytes(&[m], &[dst]).unwrap();
                let via_free =
                    hash2curve::hash_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(&[m], &[dst])
                        .unwrap();
                assert_eq!(
                    via_trait, via_free,
                    "GroupDigest::hash_from_bytes must equal the free-fn ExpandMsgXmd<Sha256> result"
                );
                assert!(via_trait.is_on_curve());
                assert!(via_trait.is_in_prime_order_subgroup());

                // `encode_from_bytes` likewise.
                let via_trait_enc = BabyJubJub::encode_from_bytes(&[m], &[dst]).unwrap();
                let via_free_enc =
                    hash2curve::encode_from_bytes::<BabyJubJub, ExpandMsgXmd<Sha256>>(&[m], &[dst])
                        .unwrap();
                assert_eq!(via_trait_enc, via_free_enc);
            }
            ...
            impl ark_ec::CurveConfig for Bjj {
                ...
                const COFACTOR_INV: BackendScalar =
                    <EdwardsConfig as ark_ec::CurveConfig>::COFACTOR_INV;
            }
            ...
            impl ark_ec::hashing::curve_maps::elligator2::Elligator2Config for Bjj {
                ...
                const COEFF_A_OVER_COEFF_B: BackendBaseField =
                    <EdwardsConfig as MontCurveConfig>::COEFF_A;
            }
        }
    }
}
